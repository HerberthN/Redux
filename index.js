...
function transacao() {
    ...
    const optIndex = Math.floor(Math.random() * (2 - 0 + 1) + 0);
    const personNameIndex = Math.floor(Math.random() * (nomes.length - 0 + 1) + 0);
    const optRandom = opt[optIndex];
    const nomeRandom = nomes[personNameIndex];

    optRandom(store, nomeRandom);
    console.log(store.getState());
